...
class DepthPipelineOutput(BaseOutput):
    ...
    intermediate_depths: Dict = None

class diffsusion_completion_pipeline(DiffusionPipeline):
    ...
    def __call__(self,
        ...
        for batch in iterable_bar:
            ...
            depth_pred_raw, max_value, min_value, intermediate_depths = self.single_infer(
                image = image,
                depth = depth,
                mask = mask,
                ...
                intermediate_step_interval = intermediate_step_interval,
                color_map = color_map,
        ...
        return DepthPipelineOutput(
            ...
            intermediate_depths=intermediate_depths if save_intermediate_steps else None
        )
    ...
    def single_infer(self,
        ...
        for i, t in iterable:
            ...
            if save_intermediate_steps and (i % intermediate_step_interval == 0 or i == len(timesteps) - 1):
                # 从latent解码得到当前深度图
                current_depth = self.decode_depth(noisy_depth_latent)
                current_depth = (current_depth + normalize_scale) / (normalize_scale*2)
                
                # 处理深度图
                current_depth_val = (current_depth * (max_value - min_value) + min_value)
                current_depth_np = current_depth_val.detach().cpu().numpy().squeeze()
                
                # 调整到原始大小
                if original_size:
                    current_depth_np = cv2.resize(current_depth_np.astype(float), original_size)
                    
                current_depth_np = current_depth_np.clip(min=0.)
                current_norm = (current_depth_np - current_depth_np.min()) / (current_depth_np.max() - current_depth_np.min())
                
                # 生成彩色深度图
                current_colored = colorize_depth_maps(
                    current_norm, 0, 1, cmap=color_map
                ).squeeze()
                current_colored = (current_colored * 255).astype(np.uint8)
                current_colored = Image.fromarray(chw2hwc(current_colored))
                
                # 保存到中间结果字典
                intermediate_depths[i] = (current_depth_np, current_colored)
        ...
        return depth, max_value, min_value, intermediate_depths
