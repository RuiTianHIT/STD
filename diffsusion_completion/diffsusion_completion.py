...
if "__main__" == __name__:
    ...
    with torch.no_grad():
        for i in tqdm(range(len(input_image_paths)), desc="Processing images"):
            ...
            depth_numpy = np.array(image)
            max_gt =depth_numpy.max()
            print(f'Max gt: {max_gt}')
            mask = np.where(depth_numpy == 0, 255, 0).astype(np.uint8)
            ...
            depth_pred=depth_pred/256.0
            max_pred = np.max(depth_pred)
            print(f'Max pred: {max_pred}')
            np.save(npy_save_path, depth_pred.astype(np.uint8))#需要修改保存数据的精度，防止占用空间过大
            print("已保存文件", npy_save_path, "是第", i + 1, "个")
            image = Image.fromarray(depth_pred.astype(np.uint8))
            image.save(os.path.join("/home/disk/tianr/ZZX/depth_completion/output/moreDens/depth", f"{pred_name_base}.png"))
